...
const App = () => {
  ...
  // localStorage: remembers UI preferences in the browser after refresh.
  ...
  const [search, setSearch] = useState(
    () => localStorage.getItem("todoSearch") || "",
  );
  const [sortBy, setSortBy] = useState(
    () => localStorage.getItem("todoSort") || "newest",
  );
  ...
  // Error handling: converts API/client errors into a message for the UI.
  ...
  // localStorage: saves the selected status filter.
  useEffect(() => {
    localStorage.setItem("todoFilter", filter);
  }, [filter]);

  // localStorage: saves the selected priority filter.
  useEffect(() => {
    localStorage.setItem("todoPriorityFilter", priorityFilter);
  }, [priorityFilter]);

  // localStorage: saves the current search text.
  useEffect(() => {
    localStorage.setItem("todoSearch", search);
  }, [search]);

  // localStorage: saves the selected sort option.
  useEffect(() => {
    localStorage.setItem("todoSort", sortBy);
  }, [sortBy]);

  // Dark mode: toggles Tailwind's dark class and saves the theme preference.
  useEffect(() => {
    document.documentElement.classList.toggle("dark", isDarkMode);
    localStorage.setItem("todoTheme", isDarkMode ? "dark" : "light");
  }, [isDarkMode]);

  // Request lifecycle: React handler calls the API service, then updates UI state.
  const handleAddTodo = async (todoData) => {
    try {
      ...
      const message = getErrorMessage(
        err,
        "Unable to add todo. Please try again.",
      );
      setError(message);
  ...
  // Request lifecycle: update request waits for the API response before replacing the todo.
  ...
  // Optimistic update + rollback: update UI first, restore previous state if API fails.
  const handleToggleDone = async (id) => {
    ...
    try {
      ...
      const message = getErrorMessage(
        err,
        "Unable to update todo status. Please try again.",
      );
      setError(message);
      showToast(message, "error");
    } finally {
  ...
  // Optimistic update + rollback: remove UI item first, restore previous state if API fails.
  ...
  const areActionsDisabled =
    isLoading || isSubmitting || Boolean(pendingAction);
  ...
  const visibleTodos = todos
    ...
    .filter(
      (todo) =>
        priorityFilter === "all" ||
        (todo.priority || "medium") === priorityFilter,
    )
